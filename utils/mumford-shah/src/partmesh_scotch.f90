...
nvisco=count(mat_domain.eq.VISCOELASTIC)
...
do ipart = 0, npart-1
  ...
  count_undef_mat,mat_domain,mat_prop,undef_mat_domain,undef_mat_prop,nvisco,  &
...
call write_ssbc(out_path,inp_path,uxfile,nelmt,part+1,npart,glob2loc_elmt(0:nelmt-1)+1)
call write_ssbc(out_path,inp_path,uyfile,nelmt,part+1,npart,glob2loc_elmt(0:nelmt-1)+1)
call write_ssbc(out_path,inp_path,uzfile,nelmt,part+1,npart,glob2loc_elmt(0:nelmt-1)+1)
write(*,'(a)')'complete!'
if(istraction)then
  ! write traction files
  write(*,'(a)',advance='no')'writing traction files...'
  call write_traction(out_path,inp_path,trfile,nelmt,part+1,npart,glob2loc_elmt(0:nelmt-1)+1)
  write(*,'(a)')'complete!'
endif
...
call detect_ghost(out_phead,nelmt,nnode,npart,max_neighbour,                   &
glob2loc_elmt(0:nelmt-1)+1) ! I need all indices starting from 1 not 0
write(*,'(a)')'complete!'
