...
use global,only:NDIM,nelmt,nelmt_solid,nnode,nndof,ngllx,nglly,ngllz,g_num,g_coord,gfile,        &
part_path,stdout
...
do i_gpart=1,ngpart ! ghost partitions loop
  ...
  !print*,'ghost:',nelmt,ubound(g_num,2),nelmt_solid
  ...
  if(myrank==1 .or. myrank==2)then
    print*,'inghost:',myrank,gpart(i_gpart)%id-1,ngelmt,gpart(i_gpart)%nnode
  endif
...
subroutine distribute2ghosts(myrank,gdof,ngpart,maxngnode,nndof,neq,ngpart_node, &
array,array_g)
use mpi
use global,only:nnode
implicit none
integer,intent(in) :: myrank,ngpart,maxngnode
integer,intent(in) :: nndof,neq
integer,dimension(nndof,nnode),intent(in) :: gdof ! global degree of freedom
! number of active ghost partitions for a node
integer,dimension(nnode),intent(in) :: ngpart_node
...
do i_gpart=1,ngpart
  ...
  ! send
  call MPI_ISSEND(send_array(:,i_gpart),ncount,MPI_KREAL,gpart(i_gpart)%id-1,  &
  tag,MPI_COMM_WORLD,send_req(i_gpart),ierr)
  ! receive
  call MPI_IRECV(recv_array(:,i_gpart),ncount,MPI_KREAL,gpart(i_gpart)%id-1,   &
  tag,MPI_COMM_WORLD,recv_req(i_gpart),ierr)
enddo
