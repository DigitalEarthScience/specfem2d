...
subroutine get_quoted_ivec(string,vname,val,n)
implicit none
character(len=*),intent(in) :: string,vname
integer,intent(in) :: n
integer,intent(out) :: val(n)

character(len=60) :: squote
integer :: i,ios,i1,i2,iv1,iv2,nlen
logical :: found

nlen=len_trim(string)
i1=index(string,vname)
i2=i1+len_trim(vname)

! find the position of first quote
found=.false.
do i=i2+1,nlen
  ...
  if(string(i:i).ne. ' ' .or. string(i:i).ne.'=')then
    write(*,*)'ERROR: properly quoted value cannot be find!',trim(vname),trim(string) 
    stop
  endif
enddo
if(.not.found)then
...
! find the position of last quote
found=.false.
do i=iv1+1,nlen
  if(string(i:i).eq.'"')then
    iv2=i-1
...
read(string(iv1:iv2),*,iostat=ios)val
if(ios.ne.0)then
  write(*,*)'ERROR: properly quoted value cannot be read!',trim(vname),trim(string) 
  stop
endif
return

end subroutine get_quoted_ivec
!=====================================================

subroutine get_quoted_rvec(string,vname,val,n)
implicit none
character(len=*),intent(in) :: string,vname
integer,intent(in) :: n
real(kind=kreal),intent(out) :: val(n)
...
end subroutine get_quoted_rvec
!=====================================================
