...
! points. the 8-noded hexahedra is conformed to the exodus/cubit numbering
! convention
subroutine shape_function_hex8p(ndim,ngnod,xi,eta,zeta,shape_hex8)
use set_precision
use math_constants
implicit none

integer,intent(in) :: ndim,ngnod
...
! 3d shape functions
real(kind=kreal) :: shape_hex8(ngnod)
...
! convention
!NOTE: dimension of dshape_hex8 is (ngnod,ndim) NOT (ndim,ngnode)
subroutine dshape_function_hex8p(ndim,ngnod,xi,eta,zeta,dshape_hex8)
use set_precision
use math_constants
implicit none
integer,intent(in) :: ndim,ngnod
...
! derivatives of the 3d shape functions
real(kind=kreal) :: dshape_hex8(ngnod,ndim)
...
! this subroutines computes the shape fucntions at gll
! points. the 4-noded quadrilateral is conformed to the exodus/cubit numbering
! convention
subroutine shape_function_quad4(ngnod,ngllx,nglly,xigll,etagll,shape_quad4)
use set_precision
use math_constants
implicit none

integer,intent(in) :: ngnod,ngllx,nglly
...
! 3d shape functions
real(kind=kreal) :: shape_quad4(ngnod,ngllx,nglly)
...
real(kind=kreal), parameter :: one_fourth = 0.25d0

! check that the parameter file is correct
if(ngnod /= 4)then
  write(*,*)'ERROR: elements must have 4 geometrical nodes!',ngnod
  stop
endif

! compute shape functions
! case of a 2d 4-node element (dhatt-touzot p. 115)
do j=1,nglly                                                                     
  etap = one + etagll(j)                                                         
  etam = one - etagll(j)                                                         
  do i=1,ngllx                                                                   
    ...
    ! corner nodes                                                               
    shape_quad4(1,i,j) = one_fourth*xim*etam                                   
    shape_quad4(2,i,j) = one_fourth*xip*etam                                   
    shape_quad4(3,i,j) = one_fourth*xip*etap                                   
    shape_quad4(4,i,j) = one_fourth*xim*etap                                   
...
do j=1,nglly
  do i=1,ngllx
    ...
    do i_gnod=1,ngnod
      sum_shape = sum_shape + shape_quad4(i_gnod,i,j)
    enddo
...
end subroutine shape_function_quad4
!=======================================================
