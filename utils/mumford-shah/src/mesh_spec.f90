...
! this subroutine convert all hexahedral meshes (8-noded) to spectral elements
! of arbitrary order defined by ngllx, nglly, and ngllz
subroutine quad2spec(ndim,ngnod,nelmt,nnode,ngllx,nglly,errcode,errtag)
use global,only : g_coord,g_num
use shape_library,only : shape_function_quad4
use gll_library, only : zwgljd
!use math_constants

implicit none
integer,intent(in) :: ndim,ngnod,ngllx,nglly
integer,intent(inout) :: nelmt,nnode
integer,intent(out) :: errcode
character(len=250),intent(out) :: errtag
integer :: ngll
real(kind=kreal),parameter :: jacobi_alpha=0.0d0,jacobi_beta=0.0d0,zero=0.0d0 !double precision
integer :: i,i_elmt,i_gnod,j
real(kind=kreal),dimension(:),allocatable :: xstore,ystore
real(kind=kreal),dimension(ngnod,ngllx,nglly) :: shape_quad4

real(kind=kreal) :: xgll,ygll
real(kind=kreal),dimension(ngllx) :: xigll,wxgll !double precision
real(kind=kreal),dimension(nglly) :: etagll,wygll !double precision
real(kind=kreal) :: xmin,xmax
...
npoint=nelmt*(ngllx*nglly)
!print*,ngllx,nglly,ngnod
!stop
allocate(xstore(npoint),ystore(npoint),stat=istat)
if(istat/=0)then
  write(errtag,'(a)')'ERROR: cannot allocate memory!'
  return
endif
...
! get shape function for 4-noded quadrilateral
call shape_function_quad4(ngnod,ngllx,nglly,xigll,etagll,shape_quad4)
...
do i_elmt=1,nelmt
  do j=1,nglly
    do i=1,ngllx
      ...
      do i_gnod=1,ngnod
        xgll = xgll + shape_quad4(i_gnod,i,j)*g_coord(1,g_num(i_gnod,i_elmt))
        ygll = ygll + shape_quad4(i_gnod,i,j)*g_coord(2,g_num(i_gnod,i_elmt))
      enddo
...
! gets ibool indexing from local (gll points) to global points
call get_global2d(ndim,nelmt,xstore,ystore,iglob,nnode,npoint,xmin,xmax)

! now we got the new number of nodes (nnode)
!- we can create a new indirect addressing to reduce cache misses
call get_global_indirect_addressing(nelmt,nnode,npoint,iglob)

allocate(g_coord(2,nnode),g_num(ngll,nelmt)) ! alocate with new number of nodes
ipoint=0
do i_elmt=1,nelmt
  ienode=0
  do j=1,nglly
...
!enddo
!close(11)
deallocate(iglob,xstore,ystore)

errcode=0
return

end subroutine quad2spec
!============================================
...
subroutine get_global2d(ndim,nelmt,xold,yold,iglob,nnode,npoint,xmin,xmax)
...
logical :: ifseg(npoint) ! initailly all are false, afterward only the unique
!ponts become true
real(kind=kreal),intent(in) :: xold(npoint),yold(npoint) !double precision
real(kind=kreal) :: xp(npoint),yp(npoint) ! double precision. initially these are the
!original coordinates of all points, at the end these become the coordinates of
!ordered points
real(kind=kreal) :: xmin,xmax
...
! sort within each segment
    ...
    do iseg=1,nseg
      ...
      call swap_all2d(iloc(ioff),xp(ioff),yp(ioff),iwork,work,ind,ninseg(iseg))
      ioff=ioff+ninseg(iseg)
    enddo
...
subroutine swap_all2d(ia,a,b,iw,w,ind,n)
!
! swap arrays ia, a, and b according to addressing in array ind
!
  implicit none
  ...
  integer :: ind(n)
  integer :: ia(n),iw(n)
  real(kind=kreal) :: a(n),b(n),w(n) !double precision
