...
type ghost_partition
  ...
  integer,dimension(:),allocatable :: inode,gdof
...
!-----------------------------------------------------------
subroutine prepare_ghost(myrank,nproc,gdof,maxngnode)
use global,only:NDIM,nelmt,nnode,NNDOF,ngllx,nglly,ngllz,g_num,g_coord,gfile,        &
part_path,stdout
...
integer,dimension(NDIM) :: ngll_vec ! (/ngllx,nglly,ngllz/) in ascending order
...
if( istat /= 0 ) then
  ...
  errcode=-1
  call error_stop(errcode,errtag,stdout,myrank)
endif
...
ofname='partition/partitioninfo'//trim(adjustl(proc_str))
...
do i_gpart=1,ngpart ! ghost partitions loop
  ...
  allocate(gpart(i_gpart)%inode(ncount),gpart(i_gpart)%isnode(ncount))
  !gpart(i_gpart)%inode=-1
  ...
  gpart(i_gpart)%inode=itmp_array(1:ncount)
  !if (myrank==2 .and. i_gpart==1)then
  !  print*,gpart_nnode(1)
  !  print*,gpart_node(1,:)
  !  print*,itmp_array
  ...
  !sort nodes
  call sort_array_coord(NDIM,ncount,xp,yp,zp,gpart(i_gpart)%inode,new_ncount)
  ...
  ! find ghost gdof
  gpart(i_gpart)%gdof=reshape(gdof(:,gpart(i_gpart)%inode),(/ncount*NNDOF/))
  !print*,gpart(i_gpart)%gdof
  !print*,'Total 0s:',myrank,ncount,count(gpart(i_gpart)%gdof==0)
  
  write(22,*)gpart(i_gpart)%nnode
  write(22,*)gpart(i_gpart)%inode

  if(myrank==1 .or. myrank==2)then
    print*,'inghost:',myrank,gpart(i_gpart)%id,ngelmt,gpart(i_gpart)%nnode
  endif
...
subroutine modify_ghost(myrank,nproc,gdof,isnode)
...
do i_gpart=1,ngpart
  ...
  !print*,'Total 0s:',myrank,ncount,count(gpart(i_gpart)%gdof==0)
  do i=1,ncount
    gpart(i_gpart)%isnode(i)=isnode(gpart(i_gpart)%inode(i))
  enddo
...
! at gdof locations
subroutine assemble_ghosts(myrank,maxngnode,NNDOF,neq,array,array_g)
!use math_library, only : maxscal_par
use mpi
implicit none
integer,intent(in) :: myrank,maxngnode
...
do i_gpart=1,ngpart
  call MPI_WAIT(send_req(i_gpart),status,ierr)
enddo
call sync_process()
array_g(0)=ZERO
return
...
! this subroutine assembles the contributions of all ghost partitions
! at nodal locations
subroutine assemble_ghosts_nodal(myrank,gdof,maxngnode,NNDOF,neq,         &
...
do i_gpart=1,ngpart
  ...
  send_array(1:ncount,i_gpart)=reshape(array(:,gpart(i_gpart)%inode),(/ncount/))
...
do i_gpart=1,ngpart
  ngnode=gpart(i_gpart)%nnode
  ncount=ngnode*NNDOF
  array_g(:,gpart(i_gpart)%inode)=array_g(:,gpart(i_gpart)%inode)+ &
...
subroutine count_active_nghosts(myrank,maxngnode,NNDOF,ngpart_node)
use mpi
use global,only:nnode
implicit none
integer,intent(in) :: myrank,maxngnode,NNDOF
! number of active ghost partitions for a node
...
do i_gpart=1,ngpart
  ...
  call MPI_ISSEND(lsend_array(:,i_gpart),ngnode,MPI_LOGICAL,gpart(i_gpart)%id,&
  tag,MPI_COMM_WORLD,send_req(i_gpart),ierr)
  ! receive
  !call MPI_IRECV(lrecv_array(1,i_gpart),ngnode,MPI_LOGICAL,gpart(i_gpart)%id-1,&
  call MPI_IRECV(lrecv_array(:,i_gpart),ngnode,MPI_LOGICAL,gpart(i_gpart)%id,&
  tag,MPI_COMM_WORLD,recv_req(i_gpart),ierr)
enddo
...
subroutine distribute2ghosts(myrank,gdof,maxngnode,NNDOF,neq,ngpart_node, &
array,array_g)
use mpi
use global,only:nnode
implicit none
...
do i_gpart=1,ngpart
  ngnode=gpart(i_gpart)%nnode
  ncount=ngnode*NNDOF
  tarray(:,gpart(i_gpart)%inode)=tarray(:,gpart(i_gpart)%inode)+ &
  reshape(recv_array(1:ncount,i_gpart),(/NNDOF,ngnode/))
  !print*,recv_array(1:ncount,i_gpart)
enddo
