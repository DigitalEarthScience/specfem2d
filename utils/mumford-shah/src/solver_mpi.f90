...
subroutine cg_solver(myid,ngpart,maxngnode,neq,nelmt,k,u_g,f,       &
...
real(kind=kreal),dimension(0:neq) :: kp,p,p_g,r,r_g,f_g
...
call assemble_ghosts(myid,ngpart,maxngnode,nndof,neq,r,r_g)
p=r
!----cg iteration----
cg: do cg_iter=1,cg_maxiter
  call assemble_ghosts(myid,ngpart,maxngnode,nndof,neq,p,p_g)
  kp=zero
  do i_elmt=1,nelmt
    egdof=gdof_elmt(:,i_elmt) !reshape(gdof(:,g_num(:,i_elmt)),(/nedof/))
    km=k(:,:,i_elmt)
  ...
  maxpg=maxvec(abs(p_g))
  maxug=maxvec(abs(u_g))
  !if(abs(alpha)*maxvec(abs(p_g))/maxvec(abs(u_g)).le.cg_tol)then
  if(abs(alpha)*maxpg/maxug.le.cg_tol)then
    errcode=0
    return
  endif
  !if(myid==1)write(*,*)cg_iter,abs(alpha)*maxpg/maxug
  r=r-alpha*kp
  call assemble_ghosts(myid,ngpart,maxngnode,nndof,neq,r,r_g)
  beta=dot_product_par(r,r_g)/rz
  p=r+beta*p
  !if(myid==1)write(*,'(i3,f25.18,f25.18,f25.18)')cg_iter,alpha,beta,rz
  !if(myid==1)write(*,*)cg_iter,alpha,beta,rz
end do cg
...
call assemble_ghosts(myid,ngpart,maxngnode,nndof,neq,z,z_g)
p=z
!----pcg iteration----
