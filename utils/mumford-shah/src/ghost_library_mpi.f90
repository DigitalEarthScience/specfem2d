...
!-----------------------------------------------------------
subroutine prepare_ghost(myid,nproc,ngpart,maxngnode)
...
!print*,'gpart allocated:',allocated(gpart)
gpart(1:ngpart)%nnode=0
gpart(1:ngpart)%id=-1
gpart(1:ngpart)%mindex=-1
!allocate(cghost(ngpart)[*]) ! it will not work here, why?!:(
write(proc_str,'(i10)')mypart !matches numbering scheme used by gindex
...
subroutine prepare_ghost_gdof(myid,ngpart,gdof)
use global,only:nnode,nndof,stdout

implicit none
integer,intent(in) :: myid,ngpart
integer,dimension(nndof,nnode),intent(in) :: gdof ! global degree of freedom
...
! this subroutine assembles the contributions of all ghost partitions
! at gdof locations
subroutine assemble_ghosts_nodal(myid,ngpart,maxngnode,nndof,array,array_g)
use mpi
...
integer,dimension(ngpart) :: send_req,recv_req
real(kind=kreal),parameter :: zero=0.0_kreal
integer :: i,j,ierr,i_gpart,ignode,ncount,ngnode
...
! this subroutine assembles the contributions of all ghost partitions
! at gdof locations
subroutine undo_unmatching_displacementBC(myid,gdof,ngpart,maxngnode,nndof,    &
bcnodalu)
use mpi
use global,only:nnode
implicit none
...
integer,dimension(ngpart) :: send_req,recv_req
real(kind=kreal),parameter :: zero=0.0_kreal
integer :: i,j,ierr,i_dof,i_node,i_gpart,igdof,ignode,ncount,ngnode
...
do i_gpart=1,ngpart
  ...
  ! send
  call MPI_ISSEND(lsend_array(:,i_gpart),ngnode,MPI_LOGICAL,gpart(i_gpart)%id-1,&
  tag,MPI_COMM_WORLD,send_req(i_gpart),ierr)
  ! receive
  call MPI_IRECV(lrecv_array(:,i_gpart),ngnode,MPI_LOGICAL,gpart(i_gpart)%id-1,&
  tag,MPI_COMM_WORLD,recv_req(i_gpart),ierr)
enddo
