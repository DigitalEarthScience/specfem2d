...
!if(myid.eq.1)print*,'Matrix norm:',mnorm
!call MatNorm(Tmat,NORM_FROBENIUS,mnorm,ierr)
!if(myid.eq.1)print*,'Symmetry norm:',mnorm
!call MatDestroy(Tmat,ierr)
!if(myid.eq.1)print*,'matrix setting & assembly complete11!'
!call sync_process
...
integer :: ncols,ncols_vali,nsparse1
PetscInt,allocatable :: cols(:),nnzero_diag(:),nnzero_offdiag(:)
integer,allocatable :: icols(:),kvec(:)
real(kind=8),allocatable :: vals(:)
real(kind=8),allocatable :: gauss(:),gkmat(:,:)
character(len=10) :: ptail
character(len=60) :: outf_name
Vec   vdiag
PetscScalar rval
...
do i_elmt=1,nelmt
  ...
  enddo
  !print*,'hey0:',num(1),maxval(abs(gkmat)),count(gkmat(1,:)>zero), &
  !gkmat(1,64).gt.gtol,gkmat(1,291).gt.gtol,gkmat(1,1111).gt.gtol,gkmat(1,1432).gt.gtol 
  ...
  !print*,'interp:',interpfgll
  !print*,'hFKl:',(kmat_m(i,i),i=1,ngll)
  gkmat=matmul(kmat_m,gkmat)
  gkmat(:,egdof)=gkmat(:,egdof)+kmat_gradm
  !print*,'hey1:',maxval(abs(gkmat)),count(gkmat(1,:)>zero), &
  !gkmat(1,64).gt.gtol,gkmat(1,291).gt.gtol,gkmat(1,1111).gt.gtol,gkmat(1,1432).gt.gtol
  !print*,'num:',num 
  ...
  do i=1,ngll
    ...
    enddo
    !print*,'bk:',num
    !print*,nnzero,'ll:',icols
    !print*,'icols:',minval(icols),maxval(icols)
    ...
    !print*,'success assembling the matrixi GLL!',i
  enddo
  !stop
enddo
